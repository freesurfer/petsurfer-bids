...
from petsurfer_km.methods import KM_METHOD_ORDER, MODEL_LABELS, MEAS_LABELS, HEMI_BIDS, MAP_FILES
    ...
    for method in methods_run:
        ...
        if vol_key in temps:
            ...
            if vol_map.exists():
                ...
                generate_volume_figure(vol_map, template_path, fig_path, meas)
        ...
        for hemi in ("lh", "rh"):
            ...
            if surf_key in temps:
                ...
                if surf_map.exists():
                    ...
                    fig_path = figures_dir / fig_name
                    generate_surface_figure(surf_map, hemi, fig_path, meas)
                    rel = f"{fig_rel_dir}/{fig_name}"
